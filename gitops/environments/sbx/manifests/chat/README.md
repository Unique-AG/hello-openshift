...
Serves the Unique chat UI at `https://<DOMAIN_BASE>/chat`.

## What this deploys

App specs (`gitops/environments/sbx/apps/`, rendered into Applications by the
`sbx` ApplicationSet), all pulling per-app charts
...
`<DOMAIN_BASE>` is already claimed by the `hubble-ui` Route in the
